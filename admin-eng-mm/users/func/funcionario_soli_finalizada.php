              ...
              <div class="col-md-12">
                <div class="card">
                  <div class="card-header">
                    <div class="card-title">Solicitações Finalizadas </div>
                  </div>
                  <div class="card-body">
                    <div class="chart-container">
                        <?php
                        // Consulta melhorada para mostrar todas as solicitações finalizadas
                        $sql = "SELECT 
                                    p.*, 
                                    c.name AS cliente_name, 
                                    t.name AS tecnico_name,
                                    DATE_FORMAT(p.data_actualizacao, '%d/%m/%Y %H:%i') as data_formatada
                                FROM pedidos p 
                                JOIN users c ON p.cliente_id = c.id 
                                LEFT JOIN users t ON p.tecnico_id = t.id
                                WHERE p.status = 'montado' 
                                ORDER BY p.data_actualizacao DESC";
                        ...
                        if($result->num_rows > 0){
                            ...
                            while($row = $result->fetch_assoc()){
                                $prioridadeClass = $row['prioridade'] == 'alta' ? 'danger' : 
                                                 ($row['prioridade'] == 'media' ? 'warning' : 'info');
                                
                                echo "<tr>
                                        <td>{$row['id']}</td>
                                        <td>{$row['cliente_name']}</td>
                                        <td>{$row['descricao']}</td>
                                        <td><span class='badge bg-{$prioridadeClass}'>{$row['prioridade']}</span></td>
